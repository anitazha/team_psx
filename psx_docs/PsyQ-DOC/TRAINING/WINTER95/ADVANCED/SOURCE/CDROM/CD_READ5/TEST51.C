...
void	cbcdsync(int intr, u_char *result);
void	cbcdready(int intr, u_char *result);

volatile int	read_stat = 0;
int		read_file_num;
volatile int	read_remain_sector;
void	*read_des_addr;
...
main()
{
	...
	while(1) {
		...
		if ((ret = check_read_file()) == CD_READ_COMPLETE) {
			...
			read_remain_sector = READ_SECTORS;
...
void cbcdsync(int intr, u_char *result)
{
	...
	else if (intr == CdlComplete) {
		switch (read_stat) {
			case	1 :		/* waiting: setting location */
				// command: set mode
				param[0] = CdlModeSpeed;
				CdControlF(CdlSetmode, param);
				read_stat++;
				break;
			case	2 :		// waiting: setting  mode
				// command: read data
				CdReadyCallback(cbcdready);
				CdControlF(CdlReadN, 0);
				read_stat++;
				break;
			case	3 :		// waiting: readn
				read_stat++;
				break;
			case	4 :		// waiting: pause
...
void cbcdready(int intr, u_char *result)
{
	if (intr == CdlDataReady) {
		CdGetSector(read_des_addr, 2048/4);
		read_des_addr += 2048;
		if (--read_remain_sector == 0) {
			CdReadyCallback(0);
			CdControlF(CdlPause, 0);
